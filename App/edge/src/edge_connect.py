...
class EdgeConnect():
    ...
    def test(self):
        ...
        for items in test_loader:
            ...
            resultimgpath = os.path.join(self.results_path, name)
            print(index, name)

            imsave(output, resultimgpath)

            # if self.debug:
            edges = self.postprocess(1 - edges)[0]
            masked = self.postprocess(images * (1 - masks) + masks)[0]
            fname, fext = name.split('.')
            edgesimgpath = os.path.join(self.results_path, fname + '_edge_'+time.strftime('%Y%m%d%H%M%S') +'.'+ fext)
            maskedimgpath = os.path.join(self.results_path, fname + '_masked_'+time.strftime('%Y%m%d%H%M%S')+'.' + fext)
            resultpathlist.append([maskedimgpath,edgesimgpath,resultimgpath])
            imsave(edges, edgesimgpath)
            imsave(masked, maskedimgpath)
