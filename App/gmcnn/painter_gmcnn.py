...
from App.gmcnn.options.gmcnn_options import TestOptions
...
class Paint(object):
    ...
    def setup(self):
        ...
        self.c.bind('<B1-Motion>', self.paint)#按下鼠标左键并移动的时候响应paint
        self.c.bind('<ButtonRelease-1>', self.reset)#鼠标释放
        self.c.bind('<Button-1>', self.beginPaint)#按下鼠标左键的时候
        self.c.bind('<Enter>', self.icon2pen)#鼠标指针进入到widget里
    ...
    def fill(self):
        ...
        print(image.shape)
        print(mask.shape)
        print("this is self mode",self.mode)
        cv2.imwrite('./imgs/inputtmp.png', image[0][:, :, :]* 1.0)
        cv2.imwrite('./imgs/masktmp.png', mask[0][:, :, :]* 1.0)
        self.result = self.sess.run(self.output, feed_dict={self.input_image_tf: image * 1.0,
                                                            self.input_mask_tf: mask * 1.0})
        cv2.imwrite('./imgs/restmp.png', self.result[0][:, :, ::-1])
        photo = Image.open('./imgs/restmp.png')
